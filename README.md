...
## 8. Fichier secrets.yml (Ansible)

Le fichier `ansible/secrets.yml` contient les secrets nécessaires au provisionnement DNS OVH et à la génération des certificats Let's Encrypt. Il n'est **pas commité** dans le dépôt Git (listé dans `.gitignore`).

> **Solution temporaire** : ce fichier est géré manuellement en attendant l'intégration d'un gestionnaire de secrets centralisé (HashiCorp Vault). En production, ces secrets seraient injectés via Vault Agent ou des variables CI chiffrées.

### Création du fichier

```bash
cat > ansible/secrets.yml << 'EOF'
## Non commit file, contains secrets, do not commit it to git
...
### Génération des credentials OVH API

Les credentials OVH sont nécessaires pour la gestion automatique des enregistrements DNS (validation ACME DNS-01 pour Let's Encrypt).

1. Se connecter sur https://api.ovh.com/createToken/
2. **Application name** : `rncp-k8s`
3. **Application description** : `Cluster K8s DNS management`
4. **Validity** : `Unlimited`
5. **Rights** : ajouter les droits suivants :
   - `GET /domain/zone/*`
   - `PUT /domain/zone/*`
   - `POST /domain/zone/*`
   - `DELETE /domain/zone/*`
6. Cliquer **Create** — noter les trois valeurs (`Application Key`, `Application Secret`, `Consumer Key`)

### Variables

| Variable | Description |
|----------|-------------|
| `ovh_application_key` | Clé d'application OVH API |
| `ovh_application_secret` | Secret d'application OVH API |
| `ovh_consumer_key` | Consumer key OVH API |
| `letsencrypt_email` | Email pour les notifications Let's Encrypt (expiration, alertes) |

---

## 9. Ordre de configuration recommandé

Pour une reconstruction complète de l'infrastructure (après redéploiement ou sur cloud backup) :

1. Créer `ansible/secrets.yml` avec les credentials OVH et l'email Let's Encrypt (section 8)
2. Attendre que tous les pods soient `Running` : `kubectl get pods -A`
3. Récupérer les mots de passe (section 1)
4. Créer le projet Harbor `poc-ci` (section 5)
5. Créer les projets SonarQube et générer le token (section 2)
6. Configurer les credentials Jenkins (section 3)
7. Configurer le serveur SonarQube dans Jenkins (section 4)
8. Lancer `./gitlab-init.sh` (section 6)
9. Créer les deux pipelines Jenkins (section 7)
